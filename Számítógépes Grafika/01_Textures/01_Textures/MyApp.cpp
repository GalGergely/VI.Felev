...
{
	// hozzuk létre a textúra pixeleit
	const int W = 128, H = 128;
	std::vector<GLubyte> tex(W * H * 3);
	for (int i = 0; i < W; ++i)
	{
		for (int j = 0; j < H; ++j)
		{
			int index = (i * H + j) * 3;
			tex[index + 0] = rand() % 256;
			tex[index + 1] = rand() % 256;
			tex[index + 2] = rand() % 256;
		}
	}
...
bool CMyApp::Init()
{
	...
	m_matProj = glm::perspective( glm::radians(60.0f), 640/480.0f, 0.01f, 1000.0f );
...
void CMyApp::Render()
{
	...
	for (int i = 0; i < n; ++i)
	{
		float distance = time + (float)(2*M_PI)/n*i;
